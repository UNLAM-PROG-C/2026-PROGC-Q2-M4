...
function getExportMeshLibraryScriptPath() {
    const dir = path.dirname(fileURLToPath(import.meta.url));
    return path.resolve(dir, "../../scripts/export_mesh_library.gd");
}
async function parseExportPresets(projectPath) {
    ...
    for (const line of lines) {
        ...
        if (key === "name")
            ...
            current.export_path = val;
    }
    if (current?.name !== undefined)
        presets.push(current);
    return presets;
}
export function registerExportTools(server, godotPath) {
    server.tool("godot_list_export_presets", "List all export presets configured in the project", {
    ...
    server.tool("godot_export_project", "Export the project using a configured export preset", {
        project_path: z.string().optional().describe("Path to the Godot project directory (auto-detected if omitted)"),
    ...
    server.tool("godot_export_mesh_library", "Export a scene as a MeshLibrary resource (for use with TileMap/GridMap)", {
        project_path: z.string().optional().describe("Path to the Godot project directory (auto-detected if omitted)"),
