 ...
 */
function openScenePaths(openScenes) {
    return (openScenes ?? []).filter((scenePath) => scenePath.length > 0);
}
async function persistConnection(bridge, signal, from, to, method, flags, remove) {
    // Get the currently open scene from editor
    const status = await bridge.send("editor.status", {});
    const openScenes = openScenePaths(status.open_scenes);
    if (openScenes.length === 0)
        return "Runtime connection only — no open scene found for persistence";
    ...
    // persist to the .tscn.
    const projectDir = status.project_path && status.project_path.length > 0
        ? status.project_path
        : resolveProjectPath();
...
export function registerSignalTools(server, bridge) {
    server.tool("godot_list_signals", "List all signals exposed by a node", {
        node_path: z.string().describe("Node path in the scene tree (empty for root)"),
    }, async ({ node_path }) => {
        try {
            const result = await bridge.send("signal.list", { path: node_path });
    ...
    server.tool("godot_connect_signal", "Connect a signal from one node to a method on another node", {
        ...
        try {
            await bridge.send("signal.connect", { from_path, signal_name, to_path, method, flags });
            let persistNote = "";
            try {
                ...
                persistNote = `Runtime only — could not persist: ${pe.message}`;
            }
    ...
    server.tool("godot_disconnect_signal", "Disconnect a signal connection between two nodes", {
        from_path: z.string().describe("Node path of the signal source"),
        ...
        try {
            await bridge.send("signal.disconnect", { from_path, signal_name, to_path, method });
            let persistNote = "";
            try {
                ...
                persistNote = `Runtime only — could not remove from file: ${pe.message}`;
            }
            return { content: [{ type: "text", text: JSON.stringify({ success: true, persisted: persistNote }, null, 2) }] };
        }
        catch (e) {
    ...
    server.tool("godot_list_connections", "List all signal connections on a node", {
        ...
        try {
            const result = await bridge.send("signal.list_connections", { path: node_path ?? "", recursive: recursive ?? true });
