...
import { listProjects, getProjectInfo, getAutoloads, addAutoload, } from "./tools/project-tools.js";
...
let cachedGodotPath = null;
async function godotPath() {
    if (!cachedGodotPath) {
...
server.tool("godot_list_projects", "List all Godot projects under a directory", {
    ...
    try {
        const projects = await listProjects(directory, recursive ?? false, sort_by, godot_version);
        return {
...
server.tool("godot_get_project_info", "Get metadata about a Godot project", {
...
server.tool("godot_launch_editor", "Launch the Godot editor for a project", {
    project_path: z.string().optional().describe("Path to the Godot project directory (auto-detected if omitted)"),
}, async ({ project_path }) => {
    try {
        const projectDir = resolveProjectPath(project_path);
...
server.tool("godot_get_uid", "Get the UID for a resource file (Godot 4.4+)", {
    project_path: z.string().optional().describe("Path to the Godot project directory (auto-detected if omitted)"),
    file_path: z
        .string()
...
server.tool("godot_update_uids", "Update all resource UIDs in a Godot project", {
    project_path: z.string().optional().describe("Path to the Godot project directory (auto-detected if omitted)"),
}, async ({ project_path }) => {
    try {
        const projectDir = resolveProjectPath(project_path);
...
// --- Autoload tools ---
server.tool("godot_get_autoloads", "List all autoload singletons configured in a Godot project", {
    project_path: z.string().optional().describe("Path to the Godot project directory (auto-detected if omitted)"),
}, async ({ project_path }) => {
    try {
        const projectDir = resolveProjectPath(project_path);
...
server.tool("godot_add_autoload", "Add an autoload singleton to a Godot project", {
    project_path: z.string().optional().describe("Path to the Godot project directory (auto-detected if omitted)"),
