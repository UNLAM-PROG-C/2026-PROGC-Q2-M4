...
export function parseAttrs(attrStr) {
    ...
    while (i < attrStr.length) {
        while (i < attrStr.length && /\s/.test(attrStr[i]))
            i++;
        const keyStart = i;
        while (i < attrStr.length && /\w/.test(attrStr[i]))
            i++;
        if (i === keyStart) {
 ...
 */
export function collectExtraAttrs(raw, known) {
    const extra = {};
...
export function serializeExtraAttrs(extra) {
    if (!extra)
        return "";
 ...
 */
export function serializeAttrs(known, extra, order) {
