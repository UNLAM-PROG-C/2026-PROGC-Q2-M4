import { collectExtraAttrs, parseAttrs, serializeAttrs, } from "./section-attrs.js";
import { parseProperties, splitSections } from "./section-io.js";
const SCENE_HEADER_ATTRS = new Set([
    "load_steps",
    "format",
    "uid",
]);
const EXT_RESOURCE_ATTRS = new Set([
    "type",
    "path",
    "id",
    "uid",
]);
const SUB_RESOURCE_ATTRS = new Set(["type", "id"]);
...
const CONNECTION_ATTRS = new Set([
    "signal",
    "from",
    "to",
    "method",
...
export function serializeSubResource(sub) {
    return ("[sub_resource" +
        serializeAttrs([
            ["type", `"${sub.type}"`],
            ["id", `"${sub.id}"`],
        ], sub.extraAttrs, sub.attrOrder) +
        "]");
}
...
export class TscnParser {
        ...
        for (const section of sections) {
            ...
            switch (tag) {
                case "gd_scene": {
                    ...
                    if (attrs.uid)
                        scene.header.uid = attrs.uid;
                    const headerExtra = collectExtraAttrs(raw, SCENE_HEADER_ATTRS);
                    if (headerExtra)
                        scene.header.extraAttrs = headerExtra;
                ...
                case "connection": {
                    ...
                    if (attrs.binds)
                        conn.binds = attrs.binds;
                    if (attrs.unbinds)
                        conn.unbinds = parseInt(attrs.unbinds, 10);
                    const extra = collectExtraAttrs(raw, CONNECTION_ATTRS);
                    if (extra)
                        conn.extraAttrs = extra;
                ...
                case "editable": {
                    if (attrs.path !== undefined)
                        scene.editables.push(attrs.path);
                    break;
                }
            }
        }
    ...
    serialize(scene) {
        ...
        lines.push("[gd_scene" +
            ...
            ], scene.header.extraAttrs, scene.header.attrOrder) +
            "]");
        if (scene.extResources.length > 0) {
            lines.push("");
            for (const ext of scene.extResources) {
                lines.push("[ext_resource" +
                    serializeAttrs([
                        ["type", `"${ext.type}"`],
                        ["path", `"${ext.path}"`],
                        ["id", `"${ext.id}"`],
                        ["uid", ext.uid ? `"${ext.uid}"` : undefined],
                    ], ext.extraAttrs, ext.attrOrder) +
                    "]");
            }
        }
        ...
        for (const node of this.sortNodes(scene.nodes)) {
            ...
            lines.push("[node" +
                ...
                ], node.extraAttrs, node.attrOrder) +
                "]");
        ...
        if (scene.connections.length > 0) {
            ...
            for (const conn of scene.connections) {
                lines.push("[connection" +
                    ...
                    ], conn.extraAttrs, conn.attrOrder) +
                    "]");
            }
        }
    ...
    addNode(scene, opts) {
        if (opts.parent === ".") {
            ...
            const parentExists = scene.nodes.some((n) => this.buildNodePath(n, scene.nodes) === fullParent);
            if (!parentExists) {
    ...
    setProperty(scene, nodePath, key, value) {
    ...
    addExtResource(scene, type, path) {
        const existing = scene.extResources.find((r) => r.type === type && r.path === path);
        if (existing)
            return { scene, id: existing.id };
        const id = `${scene.extResources.length + 1}_${randomHex(3)}`;
        const newScene = {
            ...scene,
            header: scene.header.loadSteps !== undefined
    ...
    createScene(rootNodeType, rootNodeName) {
    ...
    addConnection(scene, conn) {
        return {
            ...scene,
            connections: [...scene.connections, conn],
        };
    }
    removeConnection(scene, signal, from, to, method) {
        return {
            ...scene,
            connections: scene.connections.filter((c) => !(c.signal === signal && c.from === from && c.to === to && c.method === method)),
        };
    }
    findNodeIndex(scene, nodePath) {
        let idx = scene.nodes.findIndex((node) => this.buildNodePath(node, scene.nodes) === nodePath);
        if (idx === -1) {
            const root = scene.nodes.find((n) => n.parent === undefined);
            if (root) {
                const withRoot = `${root.name}/${nodePath}`;
                idx = scene.nodes.findIndex((node) => this.buildNodePath(node, scene.nodes) === withRoot);
            }
        }
        return idx;
    }
    getNodeByPath(scene, nodePath) {
        const idx = this.findNodeIndex(scene, nodePath);
        return idx === -1 ? undefined : scene.nodes[idx];
    }
    buildNodePath(node, allNodes) {
    ...
    resolveFullPath(allNodes, parentField) {
        const root = allNodes.find((n) => n.parent === undefined);
        if (root) {
