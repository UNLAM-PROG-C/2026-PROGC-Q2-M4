...
function macAppCandidates(dir) {
    return [
        path.join(dir, "Godot.app/Contents/MacOS/Godot"),
        ...KNOWN_GODOT_VERSIONS.map((version) => path.join(dir, `Godot_v${version}-stable_macos.universal.app/Contents/MacOS/Godot`)),
    ];
}
const PLATFORM_PATHS = {
    darwin: [
        ...
        "/usr/local/bin/godot",
        path.join(os.homedir(), "Library/Application Support/Steam/steamapps/common/Godot Engine/Godot.app/Contents/MacOS/Godot"),
    ...
    linux: [
        ...
        "/opt/godot/godot",
        path.join(os.homedir(), ".steam/steam/steamapps/common/Godot Engine/godot.x86_64"),
    ],
};
 ...
 */
async function findGodotInAppDirs() {
...
async function findGodotInUserDirs() {
    const searchDirs = [
    ...
    for (const dir of searchDirs) {
        try {
            ...
            [dir, "-maxdepth", "6", "-name", "Godot*.app", "-type", "d"], { timeout: 8000 });
...
async function validateGodot(godotPath) {
    try {
...
export async function findGodotPath() {
    ...
    if (envPath) {
        ...
        throw new Error(`GODOT_PATH is set to "${envPath}" but it is not a valid Godot executable`);
    }
    // 2. Try "godot" on PATH
    try {
    ...
    throw new Error("Could not find Godot executable. Set the GODOT_PATH environment variable or install Godot to a standard location.\n" +
